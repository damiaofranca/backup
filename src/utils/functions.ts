...
export function getStatusFranchiseeLeadText(status: any): string {
	...
	if (status_delta === FranchisseLeadStatus.TREATED)
		return FranchisseLeadStatusName.TREATED;
	if (status_delta === FranchisseLeadStatus.REJECTED)
		return FranchisseLeadStatusName.REJECTED;
	if (status_delta === FranchisseLeadStatus.NOT_ANALYZED)
		return FranchisseLeadStatusName.NOT_ANALYZED;
...
export function getStatusFranchiseeLeadColor(status: any): string {
	const status_delta = +status;
	if (status_delta === FranchisseLeadStatus.TREATED)
		return FranchisseLeadStatusColor.TREATED;
	if (status_delta === FranchisseLeadStatus.REJECTED)
		return FranchisseLeadStatusColor.REJECTED;
	if (status_delta === FranchisseLeadStatus.NOT_ANALYZED)
		return FranchisseLeadStatusColor.NOT_ANALYZED;
	return FranchisseLeadStatusColor.Unknown;
}

export function getStatusFranchisorLeadText(status: any): string {
	const status_delta = +status;
	if (status_delta === FranchisorLeadStatus.TREATED)
		return FranchisorLeadStatusName.TREATED;
	if (status_delta === FranchisorLeadStatus.REJECTED)
		return FranchisorLeadStatusName.REJECTED;
	if (status_delta === FranchisorLeadStatus.NOT_ANALYZED)
		return FranchisorLeadStatusName.NOT_ANALYZED;
...
export function getStatusFranchisorLeadColor(status: any): string {
	const status_delta = +status;
	if (status_delta === FranchisorLeadStatus.TREATED)
		return FranchisorLeadStatusColor.TREATED;
	if (status_delta === FranchisorLeadStatus.REJECTED)
		return FranchisorLeadStatusColor.REJECTED;
	if (status_delta === FranchisorLeadStatus.NOT_ANALYZED)
		return FranchisorLeadStatusColor.NOT_ANALYZED;
