...
import api from "../../../../api";
import ListPlans from "../../../../components/Products/Plans/ListPlans";
...
const DetailsProduct: React.FC = () =>
{
  ...
  const handleDelete = useCallback( async () =>
  {
    ...
    {
      notification.error( {
        message: "Ocorreu algum erro ao deletar o produto. Tente novamente., " + error,
      } );
    } finally
    {
  ...
  const onFinish = async ( values: any ) =>
  {
    ...
    {
      notification.error( {
        message: "Ocorreu algum erro ao atualizar a franquia. Tente novamente., " + error,
      } );
    } finally
    {
      ...
      <PageHeader title="Detalhes" onBack={() => history.goBack()}>
        <Tabs defaultActiveKey="details">
          <Tabs.TabPane tab="Dados gerais" key="details">
            <Form form={form} onFinish={onFinish} layout="vertical" autoComplete="off" data-testid="form-details-product-el">
              <Row gutter={16}>
                <Col md={8}>
                  <Form.Item name="name" label="Nome" rules={[{required: true, max: 512, min: 2}]}>
                    <Input placeholder="Digite o nome do produto" aria-label="name-input-el" disabled={!isEditing} />
                  </Form.Item>
                </Col>
                <Col md={8}>
              ...
              {!isEditing ? (
                <Space>
                  <Popconfirm
                    ...
                    cancelText="Cancelar"
                  >
                    <Button key="bt-prod-delete" danger color="danger" type="primary">
                      Deletar
                    </Button>
                  </Popconfirm>
        ...
        <Tabs defaultActiveKey="1">
          <Tabs.TabPane tab="Planos" key="1">
            <ListPlans productId={productId} />
          </Tabs.TabPane>
        </Tabs>
      </PageHeader>
    </>
  ...
  return <Container aria-label="container-el">{loading ? loadingTemplate : loadedTemplate}</Container>;
};
